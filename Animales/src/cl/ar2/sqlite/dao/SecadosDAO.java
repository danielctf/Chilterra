...
public class SecadosDAO {
	...
	private static final String SQL_DELETE_SYNCED = ""
			+ "DELETE FROM secado "
	...
	private static final String SQL_SELECT_GANADO_SECADO = ""
			+ "SELECT id, ganadoId, ganadoDiio, fundoId, mangada, med_control_id, "
			+ " serie, venta, sincronizado "
			+ " FROM secado "
			+ " WHERE fundoId = ? "
    ...
    public static void insertSecado(SqLiteTrx trx, List<Secado> secList) throws SQLException {
        ...
        for (Secado s : secList){
            ...
            if (s.getGan().getVenta() != null){
            	statement.bindLong(7, s.getGan().getVenta());	
            } else {
            	statement.bindNull(7);
            }
            statement.bindString(8, s.getSincronizado());
        	statement.executeInsert();
        }
    	statement.close();
    }
    ...
    public static List selectGanASincronizar(SqLiteTrx trx) throws SQLException {
        ...
        while ( hayReg ) {
        	...
        	if (!c.isNull(c.getColumnIndex("venta"))){
        		g.setVenta(c.getInt(c.getColumnIndex("venta")));
        	}
    ...
    public static void deleteSynced(SqLiteTrx trx) throws SQLException {
    	SQLiteStatement statement = trx.getDB().compileStatement(SQL_DELETE_SYNCED);
    	statement.clearBindings();
    	statement.executeUpdateDelete();
    }
    ...
    public static List selectGanadoSecado(SqLiteTrx trx, Integer fundoId) throws SQLException {
        List list = new ArrayList();
        boolean hayReg;
        
        String[] args = {Integer.toString(fundoId)};
        Cursor c = trx.getDB().rawQuery(SQL_SELECT_GANADO_SECADO, args);
        hayReg = c.moveToFirst();
        while ( hayReg ) {
        	Secado s = new Secado();
        	s.setId(c.getInt(c.getColumnIndex("id")));
