...
public class LogsRecepcion extends Activity implements View.OnClickListener, ListView.OnItemLongClickListener{
		...
		case R.id.listViewHistorial:
			ShowAlert.askYesNo("Borrar", "¿Está seguro de borrar el DIIO seleccionado?", this, new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					if (which == -2){
						try {
							Integer id = ((Ganado) arg0.getItemAtPosition(arg2)).getSqlId();
							TrasladosServicio.deleteGanadoTraslado(id);
						} catch (AppException e) {
							ShowAlert.showAlert("Error", e.getMessage(), LogsRecepcion.this);
						} finally {
							getLogs();
						}
					}
