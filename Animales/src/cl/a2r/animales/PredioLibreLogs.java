...
public class PredioLibreLogs extends Activity implements View.OnClickListener{

    private static final int SWIPE_DURATION = 250;
    private static final int MOVE_DURATION = 150;
    private boolean mSwiping = false;
    private boolean mItemPressed = false;
    private ArrayAdapterInyeccionTB mAdapter;
	...
	private void getDiiosMangada(){
    	...
    	if (stance.equals("predioLibreEncontrados")){
    		...
    		for (InyeccionTB tb : list){
    			if (tb.getMangada().intValue() == spinnerMangada.getSelectedItemPosition()){
    				listFiltrada.add(tb);
    			}
    		}
    		mAdapter = new ArrayAdapterInyeccionTB(this, android.R.layout.simple_list_item_1, listFiltrada, mTouchListener);
    ...
    private View.OnTouchListener mTouchListener = new View.OnTouchListener() {
        ...
        public boolean onTouch(final View v, MotionEvent event) {
            ...
            if (mSwipeSlop < 0) {
                mSwipeSlop = ViewConfiguration.get(PredioLibreLogs.this).
                        getScaledTouchSlop();
            }
    ...
    private void animateRemoval(final ListView listview, View viewToRemove) {
        ...
        if (mAdapter.getItem(position).getMangada().intValue() == 0){
        	ShowAlert.showAlert("Error", "No puede eliminar registros ya sincronizados", this);
        	return;
        }
        try {
			PredioLibreServicio.deletePLLog(mAdapter.getItem(position).getGanadoID());
