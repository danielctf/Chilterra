...
public class ConnectedThread extends Thread{
    ...
    public void run() {
        ...
        while (true) {
            try {
				...
				//Junta todo lo que envía el bastón en 0,2 segundos 
                if (parseData){
                	EID = EID + tempEID;
                }else{
                	EID = "";
    ...
    //--------------------------------------------------------------------------
    //--------------CONCATENACION DE LOS DATOS QUE ENVÍA EL BASTÓN--------------
    //----------------------------EN 0,2 SEGUNDOS-------------------------------
    ...
    private Runnable parseCallback = new Runnable() {
        @Override
        public void run() {
        	//Cuando se terminan los 0,2 segundos, envía al Activity correspondiente el EID.
        	parseData = false;
        	deleteChars();
