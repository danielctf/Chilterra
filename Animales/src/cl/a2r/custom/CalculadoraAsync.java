...
public class CalculadoraAsync extends Activity implements View.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_calculadora_async);
	...
	private boolean getGanadoWS(final int diio){
		...
		new AsyncTask<Void, Void, Void>(){
			...
			protected Void doInBackground(Void... params) {
				try {
					List<Ganado> list = WSGanadoCliente.traeGanado(diio);
					if (list.size() == 0){
			...
			protected void onPostExecute(Void result){
				loading.setVisibility(View.INVISIBLE);
				if (finishAct){
					CalculadoraAsync.this.finish();
				} else {
					ShowAlert.showAlert("Error", errMsg, CalculadoraAsync.this);
					diioInverso = false;
					tvdiio.setText("");
				}
			}
    ...
    private Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
    		...
    		case ConnectedThread.CONNECTION_INTERRUPTED:
    			ShowAlert.askReconnect("Error", "Se perdió la conexión con el bastón\n¿Intentar reconectar?", CalculadoraAsync.this, (BluetoothDevice) msg.obj);
    			break;
